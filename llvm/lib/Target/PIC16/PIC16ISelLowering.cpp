...
#define DEBUG_TYPE "pic16-isellower"
    ...
    : TargetLowering(TM, STI), Subtarget(STI)
{
  ...
  addRegisterClass(MVT::i32, &PIC16::GPRRegClass);
  ...
  setStackPointerRegisterToSaveRestore(PIC16::X2);
...
static const MCPhysReg GPRArgRegs[] = {
  PIC16::X0, PIC16::X1, PIC16::X2, PIC16::X3
};
...
SDValue PIC16TargetLowering::LowerFormalArguments(
  ...
  CCState CCInfo(CallConv, isVarArg, DAG.getMachineFunction(), ArgLocs,
                 *DAG.getContext());
  CCInfo.AnalyzeFormalArguments(Ins, PIC16_CCallingConv);
...
bool PIC16TargetLowering::CanLowerReturn(CallingConv::ID CallConv,
  ...
  return CCInfo.CheckReturn(Outs, PIC16_CRetConv);
}

SDValue PIC16TargetLowering::LowerCall(TargetLowering::CallLoweringInfo &CLI,
                                       SmallVectorImpl<SDValue> &InVals) const {
...
PIC16TargetLowering::LowerReturn(SDValue Chain,
  ...
  CCInfo.AnalyzeReturn(Outs, PIC16_CRetConv);
  ...
  return DAG.getNode(PIC16ISD::Ret, dl, MVT::Other, RetOps);
}
...
SDValue
PIC16TargetLowering::LowerGlobalAddress(SDValue Op, SelectionDAG &DAG) const {
  llvm_unreachable("Unsupported global address");
}

SDValue
PIC16TargetLowering::LowerBlockAddress(SDValue Op, SelectionDAG &DAG) const {
  llvm_unreachable("Unsupported block address");
}

SDValue
PIC16TargetLowering::LowerConstantPool(SDValue Op, SelectionDAG &DAG) const {
  llvm_unreachable("Unsupported constant pool");
}

SDValue
PIC16TargetLowering::LowerRETURNADDR(SDValue Op, SelectionDAG &DAG) const {
  return SDValue();
}

SDValue
PIC16TargetLowering::LowerShlParts(SDValue Op, SelectionDAG &DAG) const {
...
SDValue
PIC16TargetLowering::LowerShrParts(SDValue Op, SelectionDAG &DAG,
                                   bool arith) const {
  assert(Op.getNumOperands() == 3 && "Not a long shift");
...
SDValue
PIC16TargetLowering::LowerOperation(SDValue Op, SelectionDAG &DAG) const {
  switch (Op.getOpcode()) {
